...
#LAPC9: untreated (VEH), Condition (intact)
selected_samples <- metadata %>% dplyr::filter((Type == "HB" | Type == "LAPC9")) %>%
  dplyr::filter((is.na(treatment) | treatment == "VEH")) %>%
  dplyr::filter((is.na(Timecourse) | Timecourse == "0")) %>%
...
liana_results <- liana_wrap(sobj_specific, method = used_tools)

liana_results <- liana_wrap(sobj_specific, method = c( "logfc"))
